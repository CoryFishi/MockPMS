...
export default function Roles() {
  ...
  useEffect(() => {
    if (!rolesPulled) {
      ...
      Promise.all([getRoles(), getUsers()]).finally(() => setIsLoading(false));
    }
  ...
  return (
    <div className={`relative ${isLoading ? "overflow-hidden min-h-full" : "overflow-auto"} dark:text-white dark:bg-zinc-900 h-full`}>
      {isLoading && <LoadingSpinner loadingText="Loading roles..." />}
      {/* Edit Role Modal */}
      {isEditRoleModalOpen && (
        <EditRole
